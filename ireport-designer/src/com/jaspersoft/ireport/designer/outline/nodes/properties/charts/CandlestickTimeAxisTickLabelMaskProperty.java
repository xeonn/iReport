 ...
 */
public final class CandlestickTimeAxisTickLabelMaskProperty extends PropertySupport
{
        private final JRDesignCandlestickPlot element;
    
        @SuppressWarnings("unchecked")
        public CandlestickTimeAxisTickLabelMaskProperty(JRDesignCandlestickPlot element)
        {
            super(JRDesignCandlestickPlot.PROPERTY_TIME_AXIS_TICK_LABEL_MASK,
                    String.class, 
                    "Time Axis Tick Label Mask", "Time Axis Tick Label Mask", true, true);
            this.element = element;
            this.setValue("oneline", Boolean.TRUE);
        }
        
        public Object getValue() throws IllegalAccessException, InvocationTargetException {
            return (element.getTimeAxisTickLabelMask() == null) ? "" : element.getTimeAxisTickLabelMask();
        }

        public void setValue(Object val) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException {
            if (val instanceof String)
            {
                String oldValue = element.getTimeAxisTickLabelMask();
                String newValue = (val == null || ((String)val).trim().length() == 0) ? null : ((String)val).trim();
                element.setTimeAxisTickLabelMask(newValue);
