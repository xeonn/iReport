 ...
 */
public final class ScatterYAxisTickLabelMaskProperty extends PropertySupport
{
        private final JRDesignScatterPlot element;
    
        @SuppressWarnings("unchecked")
        public ScatterYAxisTickLabelMaskProperty(JRDesignScatterPlot element)
        {
            super(JRDesignScatterPlot.PROPERTY_Y_AXIS_TICK_LABEL_MASK,
                    String.class, 
                    "Y Axis Tick Label Mask", "Y Axis Tick Label Mask", true, true);
            this.element = element;
            this.setValue("oneline", Boolean.TRUE);
        }
        
        public Object getValue() throws IllegalAccessException, InvocationTargetException {
            return (element.getYAxisTickLabelMask() == null) ? "" : element.getYAxisTickLabelMask();
        }

        public void setValue(Object val) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException {
            if (val instanceof String)
            {
                String oldValue = element.getYAxisTickLabelMask();
                String newValue = (val == null || ((String)val).trim().length() == 0) ? null : ((String)val).trim();
                element.setYAxisTickLabelMask(newValue);
